...
namespace syncfusion.dociodemos.wpf
{
    ...
    /// </summary>
    public partial class CreateInk : DemoControl
    {
        # region Constructor
        /// <summary>
        /// Window constructor
        ...
        private void Button_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                using (WordDocument document = new WordDocument(dataPath + "CreateInkInput.docx"))
                {
                    ...
                    if (worddocx.IsChecked.Value)
                    {
                        ...
                        catch (Exception ex)
                        {
                            if (ex is IOException)
                                MessageBox.Show("Please close the file (" + Directory.GetCurrentDirectory() + "\\CreateInk.docx" + ") then try generating the document.", "File is already open",
                    ...
                    else if (pdf.IsChecked.Value)
                    {
                        ...
                        {
                            if (ex is IOException)
                                MessageBox.Show("Please close the file (" + Directory.GetCurrentDirectory() + "\\CreateInk.pdf" + ") then try generating the document.", "File is already open",
                                     MessageBoxButton.OK, MessageBoxImage.Error);
                            else
                                MessageBox.Show("Document could not be generated, Could you please email the error details to support@syncfusion.com for trouble shooting" + "\r\n" + ex.ToString(), "Error",
                                        MessageBoxButton.OK, MessageBoxImage.Error);
        ...
        private void ButtonView_Click(object sender, RoutedEventArgs e)
        {
            ...
            {
                //Opens the template document.
                string path = System.IO.Path.Combine(@"Assets\DocIO\CreateInkInput.docx");
                System.Diagnostics.Process process = new System.Diagnostics.Process();
