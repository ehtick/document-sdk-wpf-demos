...
namespace syncfusion.dociodemos.wpf
{
    ...
    /// </summary>
    public partial class WordToPDFA : DemoControl
    {
        #region Fields
                    ...
                    if (File.Exists(fullPath))
                    {
                        ...
                        using (WordDocument wordDoc = new WordDocument(fullPath))
                        {
                            try
                            {
                                ...
                                //Save the pdf file
                                pdfDoc.Save(outputFileName + "_Pdf_A3A.pdf");
                                pdfDoc.Close();
                                converter.Dispose();
                            ...
                            catch (Exception ex)
                            {
                                if (ex is IOException)
                                    MessageBox.Show("Please close the file (" + Directory.GetCurrentDirectory() + "\\" + outputFileName + "_Pdf_A3A.pdf" + ") then try generating the document.", "File is already open",
