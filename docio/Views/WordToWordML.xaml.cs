...
namespace syncfusion.dociodemos.wpf
{
    ...
    /// </summary>
    public partial class WordToWordML : DemoControl
    {
        ...
        public WordToWordML()
        {
            ...
            this.textBox1.Text = "Word to WordML.doc";
        }
        #endregion
        ...
        private void btnToWordML_Click(object sender, RoutedEventArgs e)
        {
            if (this.textBox1.Text != String.Empty && fullPath != string.Empty)
            {
                ...
                if (File.Exists(fullPath))
                {
                    try
                    {
                        //Open the document to convert from Word to WordML
                        using (WordDocument doc = new WordDocument(fullPath))
                        {
                            try
                            {
                                //Export the doc file as WordML
                                doc.Save("Word to WordML.xml", FormatType.WordML);
                                ...
                                if (MessageBox.Show("Conversion Successful. Do you want to view the WordML File?", "Status", MessageBoxButton.YesNo, MessageBoxImage.Question) == MessageBoxResult.Yes)
                                {
                                    ...
                                    process.StartInfo = new System.Diagnostics.ProcessStartInfo("Word to WordML.xml") { UseShellExecute = true };
                                    process.Start();
                                }
                            }
                            catch (Exception ex)
                            {
                                if (ex is IOException)
                                    MessageBox.Show("Please close the file (" + Directory.GetCurrentDirectory() + "\\Word to WordML.xml" + ") then try generating the document.", "File is already open",
