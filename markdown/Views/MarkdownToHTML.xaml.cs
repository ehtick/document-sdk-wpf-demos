...
namespace syncfusion.markdowndemos.wpf
{
    ...
    /// </summary>
    public partial class MarkdownToHTML : DemoControl
    {
        ...
        /// <summary>
        /// Converts the input markdown file to HTML file
        /// </summary>
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void btnToHTML_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                if (this.textBox1.Text != String.Empty && fullPath != string.Empty)
                {
                    ...
                    {
                        MessageBox.Show("Browse an Markdown file to convert to HTML", "Error", MessageBoxButton.OK, MessageBoxImage.Error);
                        return;
                    }
                    ...
                    if (File.Exists(fullPath))
                    {
                        ...
                        using (WordDocument document = new WordDocument((string)fullPath, FormatType.Markdown))
                        {
                            ...
                            try
                            {
                                ...
                                if (MessageBox.Show("Do you want to view the generated HTML File?", "Document has been created", MessageBoxButton.YesNo, MessageBoxImage.Information) == MessageBoxResult.Yes)
                                {
                                    try
                                    {
