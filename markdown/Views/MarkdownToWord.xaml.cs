...
namespace syncfusion.markdowndemos.wpf
{
    ...
    public partial class MarkdownToWord : DemoControl
    {
        ...
        /// <summary>
        /// Converts the input Markdown file to a word document
        /// </summary>
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void btnToWord_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                if (this.textBox1.Text != String.Empty && fullPath != string.Empty)
                {
                    ...
                    if (File.Exists(fullPath))
                    {
                        ...
                        using (WordDocument document = new WordDocument((string)fullPath, FormatType.Markdown))
                        {
                            ...
                            if (worddoc.IsChecked.Value)
                            {
                                ...
                                catch (Exception ex)
                                {
                                    if (ex is IOException)
                                        MessageBox.Show("Please close the file (" + Directory.GetCurrentDirectory() + "\\" + outputFileName + ".doc" + ") then try generating the document.", "File is already open",
                            ...
                            //Save as RTF format
                            else if (wordrtf.IsChecked.Value)
                            {
                                try
                                {
                                    //Saving the document as .rtf
                            ...
                            else if (wordml.IsChecked.Value)
                            {
                                try
                                {
                                ...
                                {
                                    if (ex is IOException)
                                        MessageBox.Show("Please close the file (" + Directory.GetCurrentDirectory() + "\\" + outputFileName + ".xml" + ") then try generating the document.", "File is already open",
                                             MessageBoxButton.OK, MessageBoxImage.Error);
                                    else
                                        MessageBox.Show("Document could not be generated, Could you please email the error details with along input document to support@syncfusion.com for trouble shooting" + "\r\n" + ex.ToString(), "Error",
                                                MessageBoxButton.OK, MessageBoxImage.Error);
                ...
                {
                    MessageBox.Show("Browse a Markdown document to convert to Word", "Error", MessageBoxButton.OK, MessageBoxImage.Exclamation);
                }
            }
            catch (Exception Ex)
            {
